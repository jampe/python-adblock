...
impl From<RustBlockerResult> for BlockerResult {
    fn from(br: RustBlockerResult) -> Self {
        Self {
            ...
            filter: br.filter,
            redirect_type: br.redirect.as_ref().map(|_| "url".to_string()),
...
impl BlockerResult {
    fn __repr__(&self) -> PyResult<String> {
        Ok(format!(
            "BlockerResult(matched={}, important={}, redirect={}, exception={}, filter={})",
...
fn request_err_to_py(e: ::adblock::request::RequestError) -> PyErr {
...
pub struct UrlSpecificResources {
    ...
    /// A set of procedural actions (JSON strings) that should be executed
    /// on the page. These require a ProceduralFilterer to execute.
    #[pyo3(get)]
...
impl UrlSpecificResources {
    fn __repr__(&self) -> PyResult<String> {
        Ok(format!(
            "UrlSpecificResources<{} hide selectors, {} procedural actions, {} exceptions, injected_javascript={}, generichide={}>",
            self.hide_selectors.len(),
            self.procedural_actions.len(),
            self.exceptions.len(),
...
impl Engine {
    ...
    pub fn check_network_urls(
        ...
        source_url: &str,
        request_type: &str,
    ) -> PyResult<BlockerResult> {
        let request = ::adblock::request::Request::new(url, source_url, request_type)
            .map_err(request_err_to_py)?;
        Ok(self.engine.check_network_request(&request).into())
    }
    ...
    pub fn check_network_urls_with_hostnames(
        ...
        let _ = (hostname, third_party_request);
    ...
    /// Sets this engine's resources to the provided collection.
    ///
    /// Each resource is a tuple of `(name, content_type, content, aliases)`.
    /// Note: this replaces any previously-set resources, matching the
    /// underlying `use_resources` semantics — pass all resources in one call.
    #[pyo3(signature = (resources,))]
    pub fn add_resources(
        &mut self,
        resources: Vec<(String, String, String, Option<Vec<String>>)>,
    ) -> PyResult<()> {
        let parsed: Vec<Resource> = resources
            .into_iter()
            .map(|(name, content_type, content, aliases)| Resource {
                name,
                aliases: aliases.unwrap_or_default(),
                kind: match content_type.as_str() {
                    "template" => ResourceType::Template,
                    _ => ResourceType::Mime(MimeType::from(std::borrow::Cow::from(content_type))),
                },
                content,
                dependencies: vec![],
            ...
            .collect();
        self.engine.use_resources(parsed);
        Ok(())
    }
    ...
    pub fn deserialize(&mut self, serialized: &[u8]) -> PyResult<()> {
        match self.engine.deserialize(serialized) {
            Ok(_) => Ok(()),
            Err(error) => Err(DeserializationError::new_err(format!("{error:?}"))),
        }
    }
