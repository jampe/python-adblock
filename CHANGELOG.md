...
* Update upstream `adblock` dependency to `0.12.5` (from `0.5.6`).
* `Engine.add_resource` replaced with `Engine.add_resources`, which takes a list of
  `(name, content_type, content, aliases)` tuples and sets all resources in a single
  call. This matches adblock-rust's `use_resources` API, which replaces the engine's
  resource set on each call — resources must now be provided together.
* `Engine.check_network_urls`, `check_network_urls_with_hostnames`, and
  `check_network_urls_with_hostnames_subset` now construct an internal `Request` and
  may raise on malformed URLs.

  
### Removed
* `Engine.filter_exists` — removed from adblock-rust's public API.
* `BlockerResult.error` field — removed from adblock-rust's `BlockerResult`.
* `UrlSpecificResources.style_selectors` — removed upstream (superseded by
  procedural actions).
* `include_redirect_urls` option on `FilterSet.add_filter_list` / `add_filters` —
  removed from adblock-rust's `ParseOptions`.

### Breaks
* Minimum Rust version is now `1.83` (required by PyO3 `0.28`).
* `Engine.add_resource` is replaced by `Engine.add_resources` (see Changes).
* Resource content is no longer validated at insertion time; the
  `InvalidBase64ContentError` / `InvalidUtf8ContentError` exceptions are no longer
  raised by resource insertion.
